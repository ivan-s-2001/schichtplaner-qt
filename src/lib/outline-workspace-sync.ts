...
/**
 * Synchronizes all active users, groups, and group memberships from one
 * Outline workspace into the scheduling schema.
 */
export async function syncOutlineWorkspace(
  ...
  for (const group of outlineGroups) {
    const linkedRows = await db.$queryRaw<LinkRow[]>`
    ...
    let division = linkedRows[0]
      ? await db.division.findUnique({ where: { id: linkedRows[0].id } })
      : null;
  ...
  const outlineAdminScheduleUserIds: string[] = [];
  ...
  for (const outlineUser of outlineUsers) {
    ...
    let scheduleUser = linkedRows[0]
      ? await db.user.findUnique({ where: { id: linkedRows[0].id } })
      : null;
    ...
    if (outlineUser.role === "admin") {
      outlineAdminScheduleUserIds.push(scheduleUser.id);
    }
    scheduleUserByOutlineUser.set(outlineUser.id, scheduleUser.id);
  }

  // Remove Outline-linked users from departments they no longer belong to.
  await db.$executeRaw`
  ...
  for (const membership of outlineMemberships) {
    const scheduleUserId = scheduleUserByOutlineUser.get(
      membership.outlineUserId
    );
  ...
  // Deactivate previously linked users removed or suspended in Outline.
  await db.$executeRaw`
  ...
  // Hide departments whose Outline groups were deleted while preserving history.
  await db.$executeRaw`
    UPDATE "divisions" d
  ...
  if (!owner && outlineAdminScheduleUserIds[0]) {
    await db.organizationMember.update({
      where: {
        organizationId_userId: {
          organizationId: scheduleOrganizationId,
          userId: outlineAdminScheduleUserIds[0],
        },
      },
      data: { role: "OWNER" },
    });
