...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "flex flex-col gap-4 rounded-sm border border-[var(--outline-divider)] bg-background py-4 text-foreground shadow-none",
        className
      )}
      {...props}
    />
...
function CardDescription({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "text-sm leading-5 text-[var(--outline-text-secondary)]",
        className
      )}
      {...props}
    />
